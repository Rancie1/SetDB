/**
 * SoundCloud OAuth callback page.
 * 
 * Handles the redirect from SoundCloud after user authorization.
...
const SoundCloudCallbackPage = () => {
  ...
  const { handleSoundCloudCallback, error, isAuthenticated } = useAuthStore();
  ...
  useEffect(() => {
    ...
    console.log('SoundCloud Callback Params:', params);

    // Check for OAuth error
    if (errorParam) {
      const errorMsg = errorDescription || errorParam;
      setStatus(`OAuth Error: ${errorMsg}`);
      console.error('SoundCloud OAuth Error:', errorParam, errorDescription);
    ...
    const completeLogin = async () => {
      try {
        ...
        console.log('Calling handleSoundCloudCallback with:', { code: code.substring(0, 10) + '...', state });
        const result = await handleSoundCloudCallback(code, state);
    ...
    completeLogin();
  }, [searchParams, navigate, handleSoundCloudCallback, hasProcessed]);
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center">
      <div className="max-w-md w-full bg-white rounded-lg shadow-md p-8 text-center">
        ...
        <h2 className="text-xl font-semibold text-gray-800 mb-2">SoundCloud Login</h2>
