    ...
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
          <p className="text-red-800 text-lg font-medium">{error || 'Artist not found'}</p>
          <p className="text-red-600 text-sm mt-2">
            This artist may not have been imported from Spotify yet. Try searching for one of their tracks first.
          </p>
          <Link to="/tracks" className="mt-4 inline-block text-primary-600 hover:text-primary-700 font-medium">
            Browse Tracks
          </Link>
        </div>
      </div>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex flex-col md:flex-row items-start gap-6 mb-8">
        {artist.image_url ? (
          ...
          <div className="w-40 h-40 rounded-full bg-gradient-to-br from-primary-400 to-primary-600 flex items-center justify-center text-white text-5xl font-bold shadow-lg flex-shrink-0">
        ...
        <div className="flex-1">
          ...
          {genres.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-3">
              {genres.map((genre) => (
                <span key={genre} className="px-3 py-1 bg-gray-100 text-gray-700 text-sm rounded-full">
                  {genre}
                </span>
              ))}
            </div>
          ...
          {/* Bio */}
          {!editing && artist.bio && (
            <p className="text-gray-700 whitespace-pre-wrap mb-3">{artist.bio}</p>
          )}
            ...
            <div className="mt-3 space-y-3 max-w-lg">
              ...
              <div className="flex gap-2">
                <button
                  onClick={handleSave}
                  disabled={saving}
                  className="px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white text-sm font-medium rounded-md disabled:opacity-50"
                >
                  {saving ? 'Saving...' : 'Save'}
                </button>
                <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {artist.tracks.map((track) => (
                <Link
                  key={track.id}
                  to={`/tracks/${track.id}`}
                  className="bg-white rounded-lg border border-gray-200 p-4 hover:shadow-sm hover:border-primary-200 transition-all"
                >
                  <div className="flex items-center gap-3">
                    {track.thumbnail_url && (
                      <img src={track.thumbnail_url} alt={track.track_name} className="w-12 h-12 rounded object-cover flex-shrink-0" />
                    )}
                    <div className="flex-1 min-w-0">
                      <h4 className="font-semibold text-sm text-gray-900 truncate">{track.track_name}</h4>
                      <p className="text-xs text-gray-500 truncate">{track.artist_name}</p>
                    </div>
                  </div>
                </Link>
              ))}
      ...
      {activeTab === 'sets' && (
        <div>
          {artist.sets?.length === 0 ? (
            <p className="text-gray-500 text-center py-8">No sets found for this artist.</p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {artist.sets.map((set) => (
                <Link
                ...
                >
                  <div className="flex items-center gap-3">
                    {set.thumbnail_url && (
                      <img src={set.thumbnail_url} alt={set.title} className="w-12 h-12 rounded object-cover flex-shrink-0" />
                    )}
                    <div className="flex-1 min-w-0">
                      <h4 className="font-semibold text-sm text-gray-900 truncate">{set.title}</h4>
                      <p className="text-xs text-gray-500 truncate">{set.dj_name}</p>
                    </div>
                  </div>
                </Link>
              ))}
      ...
      {activeTab === 'events' && (
        <div>
          {artist.events?.length === 0 ? (
            <p className="text-gray-500 text-center py-8">No events found for this artist.</p>
          ) : (
              ...
              {artist.events.map((event) => (
                ...
                >
                  <div className="flex-1 min-w-0">
                    ...
                    {event.dj_name && <p className="text-sm text-gray-600">{event.dj_name}</p>}
                    {event.event_date && (
                      <p className="text-xs text-gray-500 mt-1">{new Date(event.event_date).toLocaleDateString()}</p>
                    )}
                    {event.venue_location && (
                      <p className="text-xs text-gray-500">📍 {event.venue_location}</p>
                    )}
                  </div>
                </Link>
              ))}
