  ...
  if (loading) {
    return (
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        {[1, 2, 3, 4, 5].map((i) => (
          <div key={i} className="bg-surface-700 animate-pulse rounded-xl h-16" />
        ))}
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="flex flex-wrap gap-3 items-center">
        {topVenues.map((v, index) => (
          <div
            key={displayId(v)}
            className="flex items-center gap-2 bg-surface-800 border border-white/5 rounded-xl px-4 py-2"
          >
            {isOwnProfile && (
            ...
            <span className="font-medium text-slate-200">{displayName(v)}</span>
            {v.location && (
        ...
        <div className="mt-4 space-y-3">
          <div className="flex gap-2 flex-wrap items-center">
            <input
              ...
              className="px-3 py-2 bg-surface-700 border border-white/10 text-slate-100 placeholder-slate-500 rounded-xl text-sm w-48 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none"
            />
            <button
              type="button"
              onClick={runSearch}
              disabled={searching}
              className="px-3 py-2 bg-surface-700 border border-white/5 text-slate-300 rounded-xl text-sm hover:bg-surface-600 transition-colors cursor-pointer"
            >
              {searching ? 'Searching…' : 'Search'}
            </button>
          </div>
          {searchResults.length > 0 && (
            <ul className="bg-surface-700 border border-white/10 rounded-xl divide-y divide-white/5 max-h-32 overflow-y-auto">
          ...
          <form onSubmit={handleCreateAndAdd} className="flex gap-2 flex-wrap items-end">
            ...
            <input
              ...
              className="px-3 py-2 bg-surface-700 border border-white/10 text-slate-100 placeholder-slate-500 rounded-xl text-sm w-40 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none"
            />
            <button
              type="submit"
